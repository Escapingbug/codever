...
| Gateway-published native release, download, cover-install, and data preservation | Gateway admin and CVP workspace snapshot publication are enforced | The APK download/install path is enforced independently by `scripts/android-update-live-e2e.ts` on the isolated `.e2e` package |
