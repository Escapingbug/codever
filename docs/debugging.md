...
- `src/providers/coreSessionLauncher.ts` — 查询事件（query started/done/error、agent 输出、状态变更）、权限、超时、重连
...
| `[session]` | `Created CoreSession id=... provider=...` | 新 session 被创建 |
| `[session]` | `Failed to acquire creation lock` | 并发创建 session 冲突 |
| `[session]` | `Session dead, cleaned up` | Session 被销毁 |

**coreSessionLauncher 记录的事件：**

| 前缀 | 内容 | 触发时机 |
|------|------|----------|
| `[session]` | `Runner created: provider=... model=...` | Session runner 初始化 |
| `[session]` | `State: idle → querying` 等 | Session 状态机转换 |
...
示例：
```
[2026-04-24T18:30:05.000Z] [session] Created CoreSession id=abc12345 provider=opencode
[2026-04-24T18:30:05.001Z] [session] Runner created: provider=opencode model=default
