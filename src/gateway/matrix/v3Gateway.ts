...
import {
  ...
  type V3CommandJournalRecord,
  type V3CommandTerminal,
} from './fileV3CommandJournal'
import {
  FileV3RuntimeStateStore,
  type PersistedV3Project,
  type PersistedV3Session,
} from './fileV3RuntimeState'
import { V3MatrixCommandAuthorizer } from './v3Authorizer'
import { GatewayV3ContentLayer } from './v3Content'
import { materializePromptInput } from './media'
import { SessionExtensionRegistry } from '@/runtime/sessionExtensions'

interface V3SessionRuntime {
  record: PersistedV3Session
  port: V3MatrixPort
  session: TopicSession
  capabilityProvider: AgentProvider | null
  activity: { phase: CodeverV3SessionProjection['activity'] }
  activeTurnId: string | null
}

interface V3ProjectRuntime {
  config: MatrixGatewayRoomConfig
  project: PersistedV3Project
  sessions: Map<string, V3SessionRuntime>
}

type V3CommandOf<TOperation extends CodeverV3Command['operation']> = Extract<
  CodeverV3Command,
  { operation: TOperation }
>

export interface V3MatrixGatewayDependencies {
  client?: MatrixGatewayClient
  providerFactory?: (
    room: MatrixGatewayRoomConfig,
    session: Readonly<PersistedV3Session>,
...
export type V3MatrixGatewayState = 'stopped' | 'starting' | 'running' | 'stopping'

/**
 * Matrix-native v3 Gateway.
 *
 ...
 */
export class V3MatrixGatewayRunner {
  private readonly client: MatrixGatewayClient
  private readonly journal: FileV3CommandJournal
  private readonly runtimeState: FileV3RuntimeStateStore
  private readonly authorizer: V3MatrixCommandAuthorizer
  private readonly content: GatewayV3ContentLayer
  ...
  private state: V3MatrixGatewayState = 'stopped'

  constructor(
    private readonly config: MatrixGatewayConfig,
    private readonly dependencies: V3MatrixGatewayDependencies = {},
  ) {
    validateMatrixGatewayConfig(config)
    this.client = dependencies.client
      ?? createMatrixJsSdkGatewayClient(config.connection, dependencies.onLog)
    this.journal = new FileV3CommandJournal(`${config.replayLedgerPath}.v3-commands.jsonl`)
    this.runtimeState = new FileV3RuntimeStateStore(
      `${config.replayLedgerPath}.v3-runtime-state.json`,
      config.gatewayId,
    )
    this.authorizer = new V3MatrixCommandAuthorizer(config.gatewayId, this.journal)
    this.content = new GatewayV3ContentLayer(
      config.gatewayId,
  ...
  getState(): V3MatrixGatewayState {
    return this.state
  }

  async start(): Promise<void> {
    if (this.state === 'running') return
    if (this.state !== 'stopped') throw new Error(`Cannot start Matrix v3 Gateway while ${this.state}`)
    this.state = 'starting'
  ...
  async provisionCurrentState(): Promise<void> {
    if (this.state !== 'running') {
      throw new Error(`Cannot provision Matrix v3 state while Gateway is ${this.state}`)
    }
    for (const project of this.projects.values()) {
      await this.content.provisionProject(project.config, this.client)
      await this.publishProjectSnapshot(project)
  ...
  private receiveEvent(event: MatrixIncomingEvent): void {
    if (this.state === 'starting') {
      ...
      if (this.startupEvents.length >= limit) {
        this.startupFailure = new Error(`Matrix v3 startup event queue exceeded ${limit}`)
      } else {
  ...
  private enqueue(event: MatrixIncomingEvent): void {
    this.eventChain = this.eventChain
      ...
      .catch(error => {
        ...
        this.log(`[matrix-v3] rejected ${event.eventId}: ${formatError(error)}`)
      })
  }

  private async handleEvent(event: MatrixIncomingEvent): Promise<void> {
    // v3 timeline objects are standard Matrix events whose application
    ...
    if (event.sender !== opened.trustedDevice.matrixUserId) {
      throw new Error('Codever command Matrix sender does not match its device certificate')
    }
    if (
      this.dependencies.isTrustedDeviceActive
      && !(await this.dependencies.isTrustedDeviceActive(opened.authenticatedDeviceId))
    ) {
      throw new Error(`Codever device ${opened.authenticatedDeviceId} has been revoked`)
    }
    ...
    this.log(
      `[matrix-v3] command ${authorized.command.commandId} `
  ...
  private scheduleExecution(
    ...
    journalRecord: V3CommandJournalRecord,
  ): Promise<void> {
    ...
    const task = previous.catch(() => undefined).then(async () => {
      try {
        ...
        this.log(`[matrix-v3] command ${command.commandId} failed: ${formatError(error)}`)
  ...
  private async createSession(
    project: V3ProjectRuntime,
    command: V3CommandOf<'session.create'>,
    rootEventId?: string,
  ): Promise<void> {
    ...
    const record: PersistedV3Session = {
  ...
  private async executePrompt(
    project: V3ProjectRuntime,
    command: V3CommandOf<'prompt.submit'>,
  ): Promise<void> {
  ...
  private async runPrompt(
    project: V3ProjectRuntime,
    runtime: V3SessionRuntime,
    command: CodeverV3Command,
    prompt: { text: string; attachments?: import('@codever/protocol').CodeverAttachment[] },
  ): Promise<void> {
    ...
    this.log(`[matrix-v3] turn ${command.commandId} completed`)
  }

  private async cancelTurn(
    project: V3ProjectRuntime,
    command: V3CommandOf<'turn.cancel'>,
  ): Promise<void> {
    const runtime = this.requireActiveSession(project, command.sessionId)
  ...
  private async answerDecision(
    project: V3ProjectRuntime,
    command: V3CommandOf<'decision.answer'>,
  ): Promise<void> {
    const runtime = this.requireActiveSession(project, command.sessionId)
  ...
  private async updateSession(
    project: V3ProjectRuntime,
    command: V3CommandOf<'session.update'>,
  ): Promise<void> {
  ...
  private async setSessionLifecycle(
    project: V3ProjectRuntime,
    command: V3CommandOf<'session.set_lifecycle'>,
  ): Promise<void> {
  ...
  private async createInvitation(
    project: V3ProjectRuntime,
    command: V3CommandOf<'device.invitation.create'>,
  ): Promise<void> {
  ...
  private async failCommand(
    ...
    await this.settleAndDeliver(project, command, event, 'failed').catch(deliveryError => {
      this.log(`[matrix-v3] failed command result delivery failed: ${formatError(deliveryError)}`)
    })
  }
  ...
  private async settleAndDeliver(
    project: V3ProjectRuntime,
    command: CodeverV3Command,
    event: CodeverV3Event,
    outcome: V3CommandTerminal['outcome'],
    result?: JsonValue,
  ): Promise<void> {
    ...
    try {
      ...
      this.log(`[matrix-v3] terminal delivery queued: ${formatError(error)}`)
    }
  }

  private emit(
    project: V3ProjectRuntime,
    record: PersistedV3Session | undefined,
    event: CodeverV3Event,
  ) {
  ...
  private async emitBestEffort(
    ...
    try {
      ...
      this.log(`[matrix-v3] causal event delivery queued: ${formatError(error)}`)
    }
  }
  ...
  private async recoverJournal(): Promise<void> {
    ...
    for (const record of await this.journal.unfinished()) {
      const project = this.projectForRecord(record)
      if (!project) {
        this.log(`[matrix-v3] cannot recover command ${record.command.commandId}: project unavailable`)
        continue
      }
      if (record.status === 'accepted') {
        ...
        await this.settleAndDeliver(
          ...
          this.log(`[matrix-v3] interrupted command recovery failed: ${formatError(error)}`)
        })
      }
    }
  }
  ...
  private scheduleTerminalRedelivery(
    ...
    const task = this.emit(
      ...
      this.log(`[matrix-v3] terminal redelivery failed: ${formatError(error)}`)
    }).finally(() => this.executionTasks.delete(task))
    this.executionTasks.add(task)
  }

  private projectForRecord(record: V3CommandJournalRecord): V3ProjectRuntime | undefined {
  ...
  private createSessionRuntime(
    ...
    const activity = { phase: 'idle' as CodeverV3SessionProjection['activity'] }
    let runtime: V3SessionRuntime | undefined
    const port = new V3MatrixPort({
    ...
    if (this.dependencies.sessionFactory) {
      ...
      if (!provider) {
        port.close()
        throw new Error(`Matrix v3 session ${record.id} provider ${record.provider} is unavailable`)
      }
      ...
      const extensionInstances = this.extensions.createInstances(record.extensions, {
        ...
        cwd: project.project.cwd,
        providerName: record.provider,
        onLog: message => this.log(`[matrix-v3-extension] ${message}`),
      })
  ...
  private requireActiveSession(
    ...
    if (!runtime) throw new Error(`Codever session ${sessionId} is not active`)
    return runtime
  }

  private transition(
    runtime: V3SessionRuntime,
    activity: CodeverV3SessionProjection['activity'],
  ): void {
  ...
  private observeRelationHint(
    ...
    if (expected && observed && expected !== observed) {
      this.log(
        `[matrix-v3] relation hint mismatch for ${command.commandId}; `
    ...
    for (const project of projects) {
      for (const runtime of project.sessions.values()) {
        await this.destroySessionRuntime(runtime, 'shutdown').catch(error => {
          this.log(`[matrix-v3] session shutdown failed: ${formatError(error)}`)
        })
      }
    }
    await this.client.stop().catch(error => {
      this.log(`[matrix-v3] Matrix client stop failed: ${formatError(error)}`)
    })
  }
...
): CodeverV3SessionProjection['activity'] {
...
function logicalEventId(command: CodeverV3Command, stage: string): string {
...
function commandKey(command: CodeverV3Command): string {
