...
import type { GatewayV3ContentLayer } from '@/gateway/matrix/v3Content'
import type { MatrixTransport } from './transport'

export interface V3MatrixPortOptions {
  contentLayer: GatewayV3ContentLayer
  ...
  projection(): CodeverV3SessionProjection
  now?: () => number
...
/**
 * Protocol-v3 Matrix projection for one Codever session thread.
 *
 ...
 */
export class V3MatrixPort implements ChannelPort {
  ...
  constructor(private readonly options: V3MatrixPortOptions) {}
  ...
  async send(message: ChannelMessage): Promise<ChannelSendResult> {
    ...
    for (const [index, part] of parts.entries()) {
      ...
      this.options.onLog?.(
        `[matrix-v3] assistant ${logicalPartId} v${this.messageVersions.get(logicalPartId)} delivered`,
      )
    }
    return { messageId }
  }
  ...
  async edit(
    ...
    for (const [index, part] of parts.entries()) {
      ...
      this.physicalEventIds.set(logicalPartId, result.eventId)
      if (index === 0) this.physicalEventIds.set(messageId, result.eventId)
      this.options.onLog?.(`[matrix-v3] assistant ${logicalPartId} v${version} delivered`)
    }
  }
  ...
  requestDecision(request: DecisionRequest): Promise<DecisionResponse> {
    ...
    void this.options.contentLayer.sendEvent(
      ...
      this.options.onLog?.(`[matrix-v3] decision delivery failed: ${formatError(error)}`)
  ...
  notifyStatus(status: SessionStatus): void {
    try {
      ...
      this.options.onLog?.(`[matrix-v3] status observer failed: ${formatError(error)}`)
    }
  }
  ...
  sendChatAction(action: string): void {
    ...
    void this.options.transport.setTyping(
      ...
      this.options.onLog?.(`[matrix-v3] typing update failed: ${formatError(error)}`)
    })
  }
  ...
  private sendAssistantEvent(
    payload: Omit<
      Extract<CodeverV3Event['payload'], { type: 'assistant.message' }>,
      'type' | 'projection'
    > & { eventId: string },
  ...
  private baseEvent(logicalEventId: string): Omit<CodeverV3Event, 'payload'> {
    return {
      kind: 'codever.event',
      version: 3,
