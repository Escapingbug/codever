...
describe('GatewayV3ContentLayer', () => {
  it('publishes one durable key grant and one project event for every active device set', async () => {
    ...
    const layer = new GatewayV3ContentLayer(
      'workspace-1',
      {
  ...
  it('opens a client command from the same project key without comparing Matrix relations', async () => {
    ...
    const grant = codeverV3ProjectKeyGrantStateSchema.parse(state?.content)
    const plaintext = await openCodeverV3ProjectKeyGrant(grant.sealedGrant, {
      expected: {
    ...
    const mismatchedEnvelope = await sealCodeverV3Envelope({
      plaintext: { kind: 'signed_command', value: signed },
      projectKey: base64UrlDecode(projectKey.key),
      roomId: room.roomId,
      projectId: grant.projectId,
