...
import { CODEVER_MATRIX_EXTENSION, codeverV3ProjectKeyGrantStateSchema } from '@codever/protocol'
import {
  base64UrlDecode,
  exportDeviceKeyPair,
...
import { InMemoryMatrixTransport, V3MatrixPort } from '@/channel/matrix'
import { GatewayV3ContentLayer } from '@/gateway/matrix/v3Content'
import { gatewayProjectIdentity } from '@/gateway/matrix/project'

describe('V3MatrixPort', () => {
  it('projects logical message versions while treating Matrix replacement as an optional hint', async () => {
    ...
    const contentLayer = new GatewayV3ContentLayer('workspace-1', {
    ...
    const opened = await Promise.all(transport.delivered.map(async delivery => {
      ...
      return openCodeverV3Envelope(extension.envelope, {
