...
function command(id = 'command-1'): Cvp3Command {
  return {
...
function terminalEvent(): Cvp3Event {
  return {
    kind: 'codever.event',
    version: 3,
...
describe('FileCvp3CommandJournal', () => {
  it('accepts independent command IDs without a global sequence slot', async () => {
    ...
    const journal = new FileCvp3CommandJournal(path)
  ...
  it('returns the durable state for an exact duplicate across restart', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'codever-v3-journal-')), 'journal.jsonl')
    const first = new FileCvp3CommandJournal(path)
    ...
    const recovered = new FileCvp3CommandJournal(path)
    ...
    const delivered = new FileCvp3CommandJournal(path)
