import { parseGatewaySecureHandshakeFrame, parseRelaySecureAuthAcceptedPayload } from '@codever/protocol'
...
describe('one-time secure Gateway WebSocket', () => {
    it('returns a signed NATS credential only inside the OPAQUE-derived cipher', async () => {
        ...
        const app = await createRelayServer({ secureGatewayAuthenticator: authenticator })
        servers.push(app)
        ...
        socket.send(JSON.stringify({
            ...
            payload: {
                gatewayId: 'gateway-1', mode: 'pairing', subjectId: pairing.pairingId,
        ...
        if (response.type !== 'relay.secure-auth.response') throw new Error('Expected auth response')
        ...
        const acceptedFrame = parseGatewaySecureHandshakeFrame(JSON.parse(acceptedWire))
        if (acceptedFrame.type !== 'relay.secure-auth.accepted') throw new Error('Expected auth acceptance')
        ...
        expect(parseRelaySecureAuthAcceptedPayload(await cipher.decrypt(acceptedFrame.payload.envelope))).toMatchObject({
            gatewayId: 'gateway-1', natsUserJwt: jwt, natsUrl: 'tls://relay.test:4222',
        })
        socket.close()
    }, 20_000)
})
