...
<template>
  <aside class="nav-rail nav-rail--gateways">
    <RouterLink class="brand" to="/gateways" aria-label="Codever gateways"><span class="brand-mark">C</span><span>Codever</span></RouterLink>
    <div class="rail-heading"><span>Gateways</span><button class="icon-button" title="Refresh gateways" @click="state.loadGateways">↻</button></div>
    <div v-if="state.errors.gateways" class="inline-error">{{ state.errors.gateways }}</div>
    <nav class="nav-list" aria-label="Gateways">
      <RouterLink v-for="gateway in state.gateways.value" :key="gateway.id" class="nav-item" :class="{ 'nav-item--active': gateway.id === gatewayId }" :to="`/gateways/${gateway.id}`">
        <StatusDot :status="gateway.status" />
        <span class="nav-item-copy"><strong>{{ gateway.name }}</strong><small>{{ gateway.platform }} · {{ gateway.version }}</small></span>
      </RouterLink>
    </nav>
  ...
  <aside v-if="gatewayId" class="nav-rail nav-rail--sessions">
    <div class="rail-heading rail-heading--large"><div><small>Workspace</small><strong>{{ state.gateways.value.find((item) => item.id === gatewayId)?.name ?? 'Gateway' }}</strong></div></div>
    <nav class="project-list" aria-label="Projects and sessions">
      <section v-for="project in projects" :key="project.id" class="project-group">
        <RouterLink class="project-link" :class="{ 'project-link--active': project.id === projectId }" :to="`/gateways/${gatewayId}/projects/${project.id}`">
