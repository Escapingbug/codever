...
import {
  ...
  sha256,
  toArrayBuffer,
  verifyCodeverV3Pointer,
} from "@codever/security";
import type { MatrixClient, MatrixEvent, Room } from "matrix-js-sdk";
import type { RoomMessageEventContent } from "matrix-js-sdk/lib/@types/events";
import { IndexedDbMatrixV3ClientStore } from "./IndexedDbMatrixV3ClientStore";
import {
  MatrixV3ProtocolClient,
  type MatrixV3RawEvent,
} from "./matrixV3Client";
import {
...
/** Matrix SDK host for the v3 protocol core. */
export async function connectMatrixV3(
  configInput: MatrixConnectionConfig,
  handlers: V3Handlers,
): Promise<MatrixConnection> {
  const config = normalizeMatrixConfig(configInput);
  handlers.onStatus("connecting", "Opening the durable Matrix v3 client…");
  ...
  let protocol: MatrixV3ProtocolClient | null = null;
  ...
  const createProtocol = async (grantInput: unknown): Promise<void> => {
    if (!trust) return;
    const grant = codeverV3ProjectKeyGrantStateSchema.parse(grantInput);
    if (
    ...
    if (!protocol) {
      const store = new IndexedDbMatrixV3ClientStore([
        config.gatewayId,
        config.roomId,
        grant.projectId,
        identity.keyId,
      ...
      protocol = new MatrixV3ProtocolClient(
        {
        ...
        trust,
        {
          async sendMessage(request) {
            return {
              eventId: await sendMatrixV3ApplicationEvent(
                client,
                request.roomId,
        ...
        (_event, error) => {
          ...
          console.error("[matrix-v3] quarantined timeline event", error);
        },
      );
    }
  ...
  const scanGrantState = async (): Promise<boolean> => {
    ...
    for (const event of candidates) {
      const parsed = codeverV3ProjectKeyGrantStateSchema.safeParse(event.getContent());
      if (
        parsed.success
  ...
  const enqueue = (event: MatrixEvent): void => {
    inboundChain = inboundChain.then(() => ingestEvent(event)).catch(error => {
      handlers.onStatus("error", `A Matrix v3 event could not be stored: ${formatError(error)}`);
    });
  };
  ...
  const waitForGrant = async (signal?: AbortSignal): Promise<void> => {
    ...
    throw new Error("The Gateway approved pairing but did not publish the protocol-v3 project key grant.");
  };
  ...
  const loadHistory = async (sessionId: string, limit = 30): Promise<MatrixHistoryPage> => {
    await ready;
    const active = protocol;
    if (!active) throw new Error("The Codever v3 project is not initialized.");
  ...
  return {
    ...
    confirmRevisionRetry() {
      throw new Error("Protocol v3 has no global revision conflict to retry.");
    },
...
async function sendMatrixV3ApplicationEvent(
  client: MatrixClient,
  roomId: string,
...
function toIncomingMessage(
  message: import("./matrixV3Projection").V3ProjectedMessage,
...
function toLegacyCompletion(
  completion: import("./matrixV3Projection").V3CommandCompletion,
): CommandCompletion {
...
function gatewayState(
  protocol: MatrixV3ProtocolClient,
