...
export type V3ProjectedMessage = {
  ...
  partCount?: number;
  commandId?: string;
  payload?: CodeverV3Event["payload"];
};

export type V3CommandCompletion = {
  commandId: string;
...
export type MatrixV3ProjectionState = {
  ...
  completions: V3CommandCompletion[];
 ...
 */
export class MatrixV3Projection {
  ...
  private applySessionProjection(
    event: CodeverV3Event,
    next: CodeverV3SessionProjection,
    threadRootHint?: string,
  ): void {
...
function validateProjectionState(input: unknown): MatrixV3ProjectionState {
  ...
  const sessions = boundedArray(value.sessions, "sessions").map(sessionValue => {
    ...
    ) throw new Error("The Matrix v3 session projection is invalid.");
  ...
  const messages = boundedArray(value.messages, "messages").map(messageValue => {
    ...
    ) throw new Error("The Matrix v3 message projection is invalid.");
  ...
  const completions = boundedArray(value.completions, "completions").map(completionValue => {
    ...
    ) throw new Error("The Matrix v3 completion projection is invalid.");
    return {
      ...structuredClone(completion),
      event: codeverV3EventSchema.parse(completion.event),
    } as V3CommandCompletion;
  });
  const seenLogicalEvents = boundedArray(value.seenLogicalEvents, "logical events")
    .map(item => {
      if (!text(item)) throw new Error("The Matrix v3 logical event ID is invalid.");
      return item;
    });
...
function requireUnique(values: string[], name: string): void {
  if (new Set(values).size !== values.length) {
    throw new Error(`The Matrix v3 ${name} projection contains duplicate IDs.`);
  }
}

function completionFromEvent(event: CodeverV3Event): V3CommandCompletion | null {
