import {
  canonicalJsonBytes,
  cvp3ProjectKeyGrantEnvelopeSchema,
  cvp3ProjectKeyGrantPlaintextSchema,
  type Cvp3ProjectKeyGrantEnvelope,
...
export interface Cvp3KeyGrantBindings {
  grantId: string
  workspaceId: string
  projectId: string
  roomId: string
...
export async function sealCvp3ProjectKeyGrant(options: {
  plaintext: Cvp3ProjectKeyGrantPlaintext
  bindings: Cvp3KeyGrantBindings
  senderPrivateKey: CryptoKey | JsonWebKey
  recipientPublicKey: CryptoKey | JsonWebKey
}): Promise<Cvp3ProjectKeyGrantEnvelope> {
  await assertSealKeys(options)
  const plaintext = cvp3ProjectKeyGrantPlaintextSchema.parse(options.plaintext)
  ...
  return cvp3ProjectKeyGrantEnvelopeSchema.parse({
    envelope,
    signature: {
      algorithm: 'ES256',
...
export async function openCvp3ProjectKeyGrant(
  input: unknown,
  options: {
    expected: Cvp3KeyGrantBindings
    recipientPrivateKey: CryptoKey | JsonWebKey
    senderPublicKey: CryptoKey | JsonWebKey
  },
): Promise<Cvp3ProjectKeyGrantPlaintext> {
  const signed = cvp3ProjectKeyGrantEnvelopeSchema.parse(input)
  for (const field of Object.keys(options.expected) as Array<keyof Cvp3KeyGrantBindings>) {
  ...
  const plaintext = cvp3ProjectKeyGrantPlaintextSchema.parse(parsed)
