...
import { runAndroidMatrixV3Journey } from './e2e/androidMatrixV3Journey.js'
import {
    createDisposableMatrixFixture,
    type DisposableMatrixFixture,
} from './e2e/localMatrixFixture.js'

const ENABLE_ENV = 'CODEVER_MATRIX_V3_LIVE_E2E'
const REQUIRE_ANDROID_ENV = 'CODEVER_MATRIX_V3_REQUIRE_ANDROID'
...
if (process.env[REQUIRE_ANDROID_ENV] === '1' && !process.env.CODEVER_ANDROID_SERIAL) {
    throw new Error(
        'The Alpha acceptance gate requires a real Android target. Set CODEVER_ANDROID_SERIAL.',
    )
}
...
const temporaryDirectory = await mkdtemp(join(tmpdir(), 'codever-v3-live-e2e-'))
const artifactDirectory = join(repositoryRoot, 'artifacts', 'e2e', `matrix-v3-${runId}`)
...
try {
    ...
    process.stdout.write('[2/7] Building the actual PWA and starting the v3 Gateway…\n')
    await runProcess(
    ...
    gateway = managedProcess(
        ...
        repositoryRoot,
        {
            ...
            CODEVER_GATEWAY_NAME: `Codever v3 E2E ${runId}`,
    ...
    if (androidSerial) {
        process.stdout.write('[5a/7] Running the native Android v3 acceptance journey…\n')
        const androidInvitation = await admin.createInvitation({
            matrixLogin: 'disabled',
            appUrl: pwaUrl,
        })
        await runAndroidMatrixV3Journey({
            repositoryRoot,
            ...
            gatewayName: `Codever v3 E2E ${runId}`,
    ...
    const crossDevicePrompt = `v3 cross-device prompt ${runId}`
    ...
    process.stdout.write('PASS — Matrix v3 paired, created, ran concurrently, synchronized, restored, quarantined poison, and deleted.\n')
} catch (error) {
    ...
    process.stderr.write(`Matrix v3 E2E artifacts: ${artifactDirectory}\n`)
    throw error
} finally {
...
async function pairBrowser(
    ...
    // Pairing completion and the v3 key grant can independently rerender and
...
async function createSession(page: Page, model?: string): Promise<string> {
    ...
    await waitFor(async () => {
        ...
        description: 'exactly one newly created v3 session',
...
async function sendPoisonEvent(matrix: DisposableMatrixFixture): Promise<void> {
    const response = await fetch(
        ...
        + `/send/m.room.message/${encodeURIComponent(`poison-${runId}`)}`,
        {
            ...
            body: JSON.stringify({
                msgtype: 'm.notice',
                body: 'Malformed Codever v3 test event',
                'io.codever': { version: 3, envelope: { deliberately: 'invalid' } },
            }),
        },
    )
...
async function waitForConnected(page: Page): Promise<void> {
    await waitFor(() => isConnected(page), {
        description: 'connected v3 browser',
        timeoutMs: STARTUP_TIMEOUT_MS,
        failFast: () => assertNoErrors(page),
    })
    await assertNoBlockingAlerts(page)
