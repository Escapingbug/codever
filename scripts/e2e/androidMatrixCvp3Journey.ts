...
export type AndroidMatrixCvp3JourneyOptions = {
...
export async function runAndroidMatrixCvp3Journey(
    options: AndroidMatrixCvp3JourneyOptions,
): Promise<void> {
    assert.equal(
        await adb(options.serial, 'shell', 'getprop', 'ro.kernel.qemu'),
        '1',
        'The Android CVP/3 acceptance journey requires an emulator.',
    )
    const apkPath = join(
    ...
    try {
        ...
        await waitFor(async () => {
            ...
            return true
        }, 'one Android-created CVP/3 session', CONVERGENCE_TIMEOUT_MS)
        await waitFor(
        ...
        }, 'native CVP/3 restart recovery', CONNECT_TIMEOUT_MS)
        await android.clickSession(createdSessionId)
        await waitFor(async () => {
...
async function attachWebView(
    ...
    while (Date.now() < deadline) {
        ...
        try {
            ...
            if (!target?.webSocketDebuggerUrl) throw new Error('The CVP/3 WebView page is not ready.')
    ...
    throw new Error(`Timed out attaching to the Android CVP/3 WebView: ${lastError}`)
}

async function tapNativePairingConfirmation(serial: string, gatewayName: string): Promise<void> {
    const deadline = Date.now() + CONNECT_TIMEOUT_MS
    const path = '/sdcard/codever-cvp3-pairing.xml'
    while (Date.now() < deadline) {
        ...
        assert.ok(match, 'The native CVP/3 pairing dialog did not expose its Pair action.')
    ...
    throw new Error('Timed out confirming native CVP/3 pairing.')
}
